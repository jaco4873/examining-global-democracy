...
hist(testing_model$error, breaks = 15)
hist(democracy_score_predictions, breaks = 15)
```
...
# Create folds
folds <- vfold_cv(data = data_train, v = 5, strata = democracy_score, breaks = 7, pool = 0.1)
...
modelling_data %>% mutate(average = mean(democracy_score)) %>% 
  ...
  select("country", "democracy_score", "squared_error", "RMSE")
