...
data_split <- initial_split(modelling_data_norm, prop = 4/5, strata = democracy_score)
...
# Create folds
folds <- vfold_cv(data = data_train, v = 10, strata = democracy_score, breaks = 8, pool = 0.1)
...
# Generate predictions using stan_glm model on data train
```{r}
...
```
As the RMSE of the model trained on the whole training dati lies fairly close to the cross-validated RMSE mean, it seems that our model works well on unseen data.
