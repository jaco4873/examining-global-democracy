...
## Merge our final World Data Bank dataset with the freedomhouse in Python
### Prepare CSV Files for Python and open provided script
```{r}
# Write CSV files for Python
write.csv(freedomhouse_2019, "freedomhouse_2019.csv", row.names = FALSE)
write.csv(worlddatabank_oecd_eea_2019, "worlddatabank_oecd_eea_2019.csv", row.names = FALSE)

# Dirty_cat merging in Python - Open Provided Python Script
```

Merging the dataset worlddatabank_oecd_eea_2019 and freedomhouse_2019 creates issues, as the same naming conventions have not been followed on different countries.

The dirty_cat package merges the two different datasets over the columnn country, but does it in a smart way where it matches dynamically scoring a match between 0-1 after how well the strings match.

For instance, "Turkiye" from the world data bank matches quite well with "Turkey" from freedomhouse. The score is included in the match. By sorting the "score" column, we can easily inspect those matches that are not perfect, and evaluate whether to set a threshold value of matches to include, or whether it is more optimal to manually change a few country-names in the freedomhouse dataset.

### Load CSV filed written by the Python Script
```{r}
modelling_data <- read_csv("modelling_data.csv")
```
By reviewing our resulting data-set and sorting by the "score" column, it becomes obvious that four countries from our oecd_eea list of countries does not have an exact or correct match in the freedomhouse_2019 data set. 

Since it is only four countries, we have decided to manually rename those in the freedomhouse_2019:

1. "Turkiye" is named "Turkey"
2. "Czechia" is named "Czech Republic"
3. "Korea, Rep." is matched with "North Korea" and should be matched with "South Korea" 
4. "Slovak Republic" is matched with "Czech Republic" and should be matched with "Slovakia"
...
freedomhouse_2019 <- freedomhouse_2019 %>% 
  mutate(country = case_when(
    ...
    TRUE ~ country  # keep all other countries unchanged
  ))
```

We should now be able to simply merge the data sets in R:

```{r}

# Join using left join to only merge for the rows which we are interested
modelling_data <- left_join(worlddatabank_oecd_eea_2019, freedomhouse_2019, by = "country")

# Select only relevant rows:
colnames(modelling_data)
modelling_data <- modelling_data %>% 
  select(-c("year", "c_t", "region"))

# Convert into wide-format for modelling and exploration of data
modelling_data_wide<- modelling_data %>%
  pivot_wider(names_from = economic_variable, values_from = value, id_cols = c("country", "pr", "cl", "total"))
```
...
predictor_vars <- c("GDP growth (annual %)", 
...
modelling_data_wide_norm <- modelling_data_wide
modelling_data_wide_norm[predictor_vars] <- scale(modelling_data_wide_norm[predictor_vars])

# Fit the linear regression model with the normalized predictor variables
democracy_score_model_norm <- lm(total ~ 
                                  `GDP growth (annual %)` + 
                                  `GDP per capita (constant 2015 US$)`  +
                                  `Research and development expenditure (% of GDP)` +
                                  `Tax revenue (% of GDP)` +
                                  `Total natural resources rents (% of GDP)`,
                                data = modelling_data_wide_norm)
...
democracy_score_model_gdp <- lm(total ~ `GDP per capita (constant 2015 US$)`,
                                 data = modelling_data_wide_norm)
...
democracy_score_model_tax_revenue <- lm(total ~ `Tax revenue (% of GDP)`,
                                 data = modelling_data_wide_norm)
...
```{r}
ggplot(modelling_data_wide, aes(x = total)) + # map the democracy score variable
  geom_histogram(fill = "steelblue") +  # create histogram with fill color
  geom_vline(aes(xintercept = mean(total)), linetype = 2) + #creating dashed line at x=0
  theme_minimal() + 
  ylab("Percent-points") + 
...
### Histogram
```{r}
ggplot(modelling_data_wide, aes(x = `GDP growth (annual %)`)) + 
  geom_histogram(fill = "steelblue") +  
  geom_vline(aes(xintercept = mean(`GDP growth (annual %)`)), linetype = 2) + 
  theme_minimal() + 
...
```{r}
ggplot(modelling_data_wide_norm, aes(x = `GDP growth (annual %)`, y = total)) +
  geom_point() +
...
### Histogram
```{r}
ggplot(modelling_data_wide, aes(x = `GDP per capita (constant 2015 US$)`)) + 
  geom_histogram(fill = "steelblue") +  
  geom_vline(aes(xintercept = mean(`GDP per capita (constant 2015 US$)`)), linetype = 2) + 
  theme_minimal() + 
  ylab("Number of countries")
```
### Scatterplot
...
```{r}
ggplot(modelling_data_wide_norm, aes(x = `GDP per capita (constant 2015 US$)`, y = total)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
### Histogram
```{r}
ggplot(modelling_data_wide, aes(x = `Research and development expenditure (% of GDP)`)) + 
  geom_histogram(fill = "steelblue") +  
  geom_vline(aes(xintercept = mean(`Research and development expenditure (% of GDP)`)), linetype = 2) + 
  theme_minimal() + 
  ylab("Number of countries")
```
...
```{r}
ggplot(modelling_data_wide_norm, aes(x = `Research and development expenditure (% of GDP)`, y = total)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
### Histogram
```{r}
ggplot(modelling_data_wide, aes(x = `Tax revenue (% of GDP)`)) + 
  geom_histogram(fill = "steelblue") +  
  geom_vline(aes(xintercept = mean(`Tax revenue (% of GDP)`)), linetype = 2) + 
  theme_minimal() + 
  ylab("Number of countries")
```
### Scatterplot
...
```{r}
ggplot(modelling_data_wide_norm, aes(x = `Tax revenue (% of GDP)`, y = total)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
### Histogram 
```{r}
ggplot(modelling_data_wide, aes(x = `Total natural resources rents (% of GDP)`)) + 
  geom_histogram(fill = "steelblue") +  
  geom_vline(aes(xintercept = mean(`Total natural resources rents (% of GDP)`)), linetype = 2) + 
  theme_minimal() + 
  ylab("Number of countries")
```
### Scatterplot
```{r}
ggplot(modelling_data_wide_norm, aes(x = `Total natural resources rents (% of GDP)`, y = total)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
