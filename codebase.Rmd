...
worlddatabank <- read_csv(file.choose(), show_col_types = FALSE) %>%  
  ...
  mutate_at(vars(5:13), as.numeric) %>%          # convert observations to decimals
...
worlddatabank2 <- worlddatabank[, c("country", "variable")]
...
worlddatabank2 <- cbind(worlddatabank2, worlddatabank[, 3:ncol(worlddatabank)])
...
missing_data_function(worlddatabank2)
```
...
## Subset final dataset to only include 2019 data for all countries excluding the Gini-variable and R&D
```{r}
...
worlddatabank_2019 <-  worlddatabank_2019 %>%
  pivot_wider(names_from = variable, values_from = "2019", id_cols = "country") %>% 
...
```{r}
# Create a diagnose statistics
diagnose_web_report(modelling_data)
diagnose(modelling_data)
diagnose_numeric(modelling_data)
diagnose_outlier(modelling_data) 
plot_outlier(modelling_data)
```
# Explore data
```{r}
describe(modelling_data)
normality(modelling_data)
plot_normality(modelling_data) # consider log-transforming GDP pr capita

# Check all variable correlations numerically and visually
correlate(modelling_data) %>% flextable()
plot_correlate(modelling_data) 
```
...
# Create model based on normalized predictors
```{r}
...
We can then fit the linear regression model using stan_glm
```{r}
...
democracy_score_model_norm <- stan_glm(democracy_score ~ 
                                  ...
                                  tax_revenue_percent_of_gdp +
                                  total_natural_resources_rents_percent_of_gdp,
                                data = data_train, refresh = 0)
# Inspect model
print(democracy_score_model_norm)
```

# Generate predictions using the linear model for the test data and compare to true observations
```{r}

# Generate point predictions
democracy_score_predictions <- predict(object = democracy_score_model_norm, newdata = data_test)

# Check the prediction accuracy
testing_model <- data_test %>% na.omit(data_test) %>% 
  add_column(democracy_score_predictions) %>% 
  mutate(error = democracy_score-democracy_score_predictions) %>% 
  mutate(squared_error = round(error^2, digits = 3)) %>% 
  select("country", "democracy_score", "democracy_score_predictions", "error", "squared_error")

print(testing_model)

# CALCULATING RMSE of predictions made by the model based on training dataset
sqrt(mean(testing_model$squared_error))

# Histogram of errors in predictions
ggplot(testing_model, aes(x = error)) + 
  geom_histogram(fill = "steelblue", binwidth = 15) +  
  geom_vline(aes(xintercept = mean(error)), linetype = 2) +
  theme_minimal() + 
  xlab("Democracy Score Prediction Residuals") +
  ggtitle("Model Residuals")
```


# Cross validate model using stratified 10-fold cross validation
```{r}
...
data_split <- initial_split(modelling_data_norm, prop = 7/10, strata = democracy_score)
data_train <- training(data_split)
data_test <- testing(data_split)

# Create receipe
data_recipe <- recipe(democracy_score ~ 
                              gdp_growth_annual_percent + 
                              gdp_per_capita_constant_2015_us  +
                              tax_revenue_percent_of_gdp +
                              total_natural_resources_rents_percent_of_gdp, 
...
# Create folds
folds <- vfold_cv(data = data_train, v = 5, strata = democracy_score, pool = 0.1)
?vfold_cv
...
cv_results
```
As the RMSE of the model trained on the whole training data lies fairly close to the cross-validated RMSE mean, it seems that our model generalizes well outside the sample.


# Comparing to intercept-only model
```{r}
modelling_data %>% mutate(average = mean(democracy_score)) %>% 
  mutate(squared_error = ((democracy_score-average)^2)) %>% 
  mutate(MSE = mean(squared_error)) %>% 
  mutate(RMSE = sqrt(MSE)) %>% 
  select("country", "average", "democracy_score", "squared_error", "RMSE")

```
Our cross validated model obtains a much smaller RMSE for predictions than if we were just guessing a country's democracy score by the average of all countries.

We can furthermore see, that tere is an improvement from roughly 30 RMSE to 20 RMSE using the model.
...
```{r}
# General check 
check_model(democracy_score_model_norm, check = c("qq", "linearity", "homogeneity", "pp_check"))

# Collinearity (VIF)
plot(check_collinearity(democracy_score_model_norm))
