...
freedomhouse <- freedomhouse %>% 
  ...
  rename(country = country_territory, year = edition ) %>%     # rename column s
...
worlddatabank <- read_csv("worlddatabank.csv",show_col_types = FALSE) %>% 
  rename_at(vars(5:14), ~ substr(., 1, 4)) %>%   # rename "year"-columns
  mutate_at(vars(5:14), as.integer) %>%          # convert observations to integers
  select(-c(2, 4, 5, 14)) %>%                           # remove irrelevant columns
...
worlddatabank_long <- pivot_longer(worlddatabank, 
  cols = 3:10, names_to = "year", values_to = "value") %>% 
  mutate(year = as.integer(year))
write.csv(worlddatabank, "worlddatabank_clean_long.csv", row.names = FALSE)

# Merge data: Freedom House Scores and Economic variables
merged_data <- left_join(worlddatabank_long, freedomhouse, by = c("year", "country")) %>% 
  relocate(region, .after = country) %>% 
  relocate(value, .after = total) 
